...
def gen_xs_estados(df, sector):
    df = df[df["desc"] == sector]

    top = (df
           .sort_values("t2022", ascending=False)
           .loc[:,["estado","t2022"]]
        )
    
    tt = df["t2022"].sum()
    ttop = top["t2022"].head(10).sum()
    tt = round((ttop/tt)*100)

    return top, tt

def fig_bar_xs(df, sector):
    df, ratio = gen_xs_estados(df, sector)
    df = df.head(10)
    
    fig = go.Figure(go.Bar(x = df["t2022"], y = df["estado"],
        marker=dict(color = "rgb(188, 149, 92)"),
        texttemplate = "%{x:,.0f}",
        textposition='inside',
        orientation = "h",
