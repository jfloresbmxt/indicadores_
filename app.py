...
@st.cache_data
def get_info():
    sectores = pd.read_excel("data/sectorial/sectorial_pib_v2.xlsx", dtype={"sector":str})
    ...
    pib = pd.read_excel("data/sectorial/sectorial_pib_v2.xlsx").fillna(0)

    xs = pd.read_excel("data/sectorial/sectorial_xs_v2.xlsx")
    xs = xs.merge(sectores, on="sector", how = "right").fillna(0)

    ied = pd.read_excel("data/sectorial/sectorial_ied_v2.xlsx", dtype={"sector":str})
    ied = ied.merge(sectores, on="sector")

    ied_estatal = pd.read_excel("data/estatal/estatales_ied.xlsx")
    ied_estatal = ied_estatal.merge(sectores, on="sector")

    estatal = pd.read_excel("data/estatal/estatales_pib.xlsx").fillna(0)
    ...
    xs_estatal = pd.read_excel("data/estatal/estatales_xs.xlsx")
    ...
    vaemg = pd.read_excel("data/sectorial/sectorial_vaemg_v2.xlsx", dtype={"sector":str})
